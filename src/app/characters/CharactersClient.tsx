...
export default function CharactersClient({ initialCharacters, initialTotal, availableTags }: Props) {
  const dbTags = new Set(availableTags)
  ...
  const load = useCallback(async () => {
    if (page === 1 && debouncedSearch === '' && selectedTag === '') {
    ...
    setTotal(total)
    setLoading(false)
  }, [page, debouncedSearch, selectedTag, initialCharacters, initialTotal])
    ...
    <div>
      <div className="bg-gradient-to-r from-pink-50 to-white border-b border-gray-100 py-5 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6">
          <h1 className="text-lg font-bold text-gray-800 flex-shrink-0">キャラクター一覧</h1>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="flex items-center justify-end gap-3 mb-4">
          {(selectedTag || searchInput) && (
            <button onClick={() => { setSelectedTag(''); setSearchInput('') }} className="text-xs text-gray-400 hover:text-pink-500 transition-colors flex items-center gap-1">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-2">
            <p className="text-gray-400">条件に一致するキャラクターが見つかりませんでした</p>
            <button onClick={() => { setSelectedTag(''); setSearchInput('') }} className="text-sm text-pink-500 hover:underline">条件をクリア</button>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {characters.map((character) => <CharacterCard key={character.character_id} character={character} />)}
            </div>
